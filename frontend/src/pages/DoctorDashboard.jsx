...
const DoctorDashboard = () => {
  ...
  const handleCreateLeave = async () => {
    ...
    try {
      ...
      setLeaveData({ startDate: new Date().toISOString().split('T')[0], endDate: '', reason: '' });
    } catch (error) {
    ...
    <div className="min-h-screen bg-sky-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500">Welcome, Dr. {user?.name?.replace('Dr. ', '')}</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-sky-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-800">{queueStats.total}</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-red-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-red-600">{queueStats.red}</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-orange-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-orange-600">{queueStats.orange}</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-green-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-green-600">{queueStats.green}</p>
                  ...
                  {queue.map((patient) => (
                    <button
                      ...
                      className={`w-full p-4 text-left hover:bg-sky-50 transition-colors ${
                        selectedPatient?._id === patient._id ? 'bg-sky-50' : ''
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${getSeverityColor(patient.severity)}`}></div>
                  ...
                  {detailsLoading ? (
                    ...
                    <>
                      ...
                      <div className="bg-sky-50 rounded-lg p-4">
                        <p className="text-sm font-medium text-gray-700 mb-2">Severity Classification</p>
                        <div className="flex gap-2">
                          {['red', 'orange', 'green'].map((sev) => (
                            ...
                            >
                              {sev === 'red' ? 'Critical' : sev === 'orange' ? 'Moderate' : 'Normal'}
                            </button>
                          ))}
                        </div>
                        <p className="text-xs text-gray-500 mt-2">
                          ⚠️ Severity is decision-support only, not final diagnosis
                        </p>
                      </div>
                      ...
                      {/* Vitals */}
                      {selectedPatient.vitals && Object.keys(selectedPatient.vitals).length > 0 && (
                        <div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            {selectedPatient.vitals.oxygenLevel && (
                              <div className="bg-gray-50 p-3 rounded-lg">
                                ...
                                <p className="font-semibold">{selectedPatient.vitals.oxygenLevel}%</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {patientDetails?.history?.length > 0 && (
                        <div>
                          <p className="text-sm font-medium text-gray-700 mb-2">Previous Visits</p>
                          <div className="space-y-2">
                            {patientDetails.history.slice(0, 3).map((h, i) => (
                              <div key={i} className="bg-gray-50 p-3 rounded-lg text-sm">
                                <div className="flex justify-between">
                                  <span className="text-gray-600">{h.symptoms.join(', ')}</span>
                      ...
                      {suggestedTests.length > 0 && (
                        <div>
                          <p className="text-sm font-medium text-gray-700 mb-2">Suggested Tests</p>
                          <div className="flex flex-wrap gap-2">
        ...
        {showLeaveForm && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">Create Medical Leave</h3>
        ...
        {showDietForm && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">Create Diet Plan</h3>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Diet Type</label>
                  <select
                    value={dietData.dietType}
                    onChange={(e) => setDietData({...dietData, dietType: e.target.value})}
                    className="w-full px-3 py-2 border border-sky-200 rounded-lg"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setDietData({...dietData, endDate: e.target.value})}
                    className="w-full px-3 py-2 border border-sky-200 rounded-lg"
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setDietData({...dietData, specialInstructions: e.target.value})}
                    rows={3}
                    className="w-full px-3 py-2 border border-sky-200 rounded-lg"
